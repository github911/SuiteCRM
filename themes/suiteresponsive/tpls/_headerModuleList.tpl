    ...
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
            <ul class="nav navbar-nav">
                {foreach from=$moduleTopMenu item=module key=name name=moduleList} {if $name == $MODULE_TAB}
                <li id="topnav">
				...
				<span class="breaker">
				{foreach from=$shortcutTopMenu.$name item=item} {if $item.URL == "-"}
                    <li><a></a><span>&nbsp;</span></li>
				 {else}
				<li><a href="{$item.URL}" class=""><span>{$item.LABEL}</span></a></li>
                {/if} {/foreach} <br>
                    {/if} {if $name == 'Home' and !$lock_homepage}
				...
				<br>
                    {foreach from=$recentRecords item=item name=lastViewed} {if $name == 'Home'}
                    <table style="width:100%">
                        <tr>
                            <td>
                                <li>
                                    <a title="{$item.module_name}" accesskey="{$smarty.foreach.lastViewed.iteration}" href="{sugar_link module=$item.module_name action='DetailView' record=$item.item_id link_only=1}">
            ...
            {/if} {foreachelse} {$APP.NTC_NO_ITEMS_DISPLAY} {/foreach}
            </ul>
            {else}
            <li id="topnav">
        ...
        {/if} {foreachelse} {$APP.NTC_NO_ITEMS_DISPLAY} {/foreach}
            ...
            <div id="globalLinks" class="dropdown nav navbar-nav navbar-right">
                <li id="usermenu" class="dropdown-toggle" data-toggle="dropdown" aria-expanded="true">
                    <span class="glyphicon glyphicon-user"></span> {$CURRENT_USER} <span class="caret"></span>
                </li>
    ...
    <div class="row">
        <div class="col-sm-3 col-md-2 sidebar">
            <div class="btn-toolbar" role="toolbar" aria-label="...">
                ...
                <div class="btn-group" role="group" aria-label="...">
                    <a role="menuitem" id="logout_link" href='{$LOGOUT_LINK}' class="btn btn-success"><span class=" glyphicon glyphicon-log-out" aria-hidden="true"></span></a>
                </div>
            </div>
            <br>
            <div class="dropdown">
